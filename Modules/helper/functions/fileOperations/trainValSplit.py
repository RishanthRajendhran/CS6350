...
def trainValSplit(X, weights, Y, validationSplit=0.3, shuffle=False):
    ...
    if shuffle:
        train_WXY = np.zeros((train_X.shape[0], 1+train_X.shape[1]+1)).astype(str)
        train_WXY[:,0] =train_W.copy()
        train_WXY[:,1:-1] = train_X.copy()
        train_WXY[:,-1] = train_Y.copy()
        np.random.shuffle(train_WXY)
        train_W = np.copy(train_WXY[:,0])
        train_X = np.copy(train_WXY[:,1:-1])
        train_Y = np.copy(train_WXY[:,-1])
